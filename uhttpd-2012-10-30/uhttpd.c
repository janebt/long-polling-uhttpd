                          ...
                          struct config *conf)
{
	...
	for (p = addrs; p; p = p->ai_next)
	{
		...
		/* listen */
		if (listen(sock, conf->max_requests) == -1)
		{
			perror("listen()");
			goto error;
		}
...
#ifdef REMOTE_MANAGE
		/* work == 1 means listener will listen client request, otherwise not. */
		l->work = 1;
		if (bind_wan)
		{
			conf->wan_listen_success = 1;
			conf->wan_listener = l;
		}
#endif
...
static bool uh_path_check(const char *url)
{
	char *ptr = NULL;
...
static bool uh_dispatch_request(struct client *cl, struct http_request *req)
{
	...
	if (!uh_path_check(req->url))
	{
		D("SRV: URL[%s] include relative path chars, invlid.\n", req->url);
...
#ifdef HAVE_LUA
	if (conf->lua_state &&
		0 != strncmp(req->url, SLP_CGI_BIN_LUCI, strlen(SLP_CGI_BIN_LUCI))
		&& 0 != strncmp(req->url, SLP_LUCI_STATIC, strlen(SLP_LUCI_STATIC)))
	{
...
static void uh_listener_cb(struct uloop_fd *u, unsigned int events)
{
	...
	if (
		...
		(0 == serv->work) ||
#endif
		(serv->n_clients >= conf->max_requests))
	{
		sleep(1);
		return;
	}
...
#ifdef REMOTE_MANAGE
/******************************************************************************
* FUNCTION		: get_wan_conf()
* AUTHOR		: wangfuyu <wangfuyu@tp-link.net>
* DESCRIPTION	: get wan device configure info
* INPUT			:
* OUTPUT		:
* RETURN		:
******************************************************************************/
static int get_wan_conf(WAN_CONF_INFO *wan_conf)
{
	int ret = -1;
	char *dev_name = NULL;
	char *wan_proto = NULL;
	...
	if (!ctx)
	{
		D("uci alloc context failed.\n");
		return -1;
	}

	/* get uci section: uhttpd.main */
	uci_section = get_uci_section(ctx, SLP_UHTTPD_PACKAGE_NAME, SLP_UHTTPD_MAIN_SECT_NAME);
	if (NULL == uci_section)
	{
		D("find section(%s) failed\n", SLP_UHTTPD_MAIN_SECT_NAME);
		goto wan_error;
	}

	/* get uci option: uhttpd.main.listen_http_wan */
	listen_str = lookup_uci_option_str(ctx, uci_section, SLP_UHTTPD_WAN_PORT_OPT);
	if (NULL == listen_str)
	{
		D("get option(%s) failed\n", SLP_UHTTPD_WAN_PORT_OPT);
		goto wan_error;
	}

	if ((listen_port = strrchr(listen_str, ':')) != NULL)
	{
		if ((listen_str[0] == '[') && (listen_port > listen_str) && (listen_port[-1] == ']'))
			memcpy(wan_conf->listen_host, listen_str + 1,
				min(SLP_WAN_LISTEN_HOST_MAX_LEN, (int)(listen_port - listen_str) - 2));
		else
			memcpy(wan_conf->listen_host, listen_str,
				min(SLP_WAN_LISTEN_HOST_MAX_LEN, (int)(listen_port - listen_str)));
	...
	strncpy(wan_conf->listen_port, listen_port ? listen_port : "", SLP_WAN_LISTEN_PORT_MAX_LEN);

	/* get uci section: network.wan */
	uci_section = get_uci_section(ctx, SLP_NETWORK_PACKAGE_NAME, SLP_NETWORK_WAN_SECT_NAME);
	if (NULL == uci_section)
	{
		D("find section(%s) failed\n", SLP_NETWORK_WAN_SECT_NAME);
		goto wan_error;
	}

	/* get uci option: network.wan.proto && network.wan.ifname */
	wan_proto = lookup_uci_option_str(ctx, uci_section, SLP_NETWORK_WAN_PROTO_OPT);
	if (0 == strncmp(wan_proto, SLP_WAN_PROTO_DHCP, strlen(SLP_WAN_PROTO_DHCP))
		|| 0 == strncmp(wan_proto, SLP_WAN_PROTO_STATIC, strlen(SLP_WAN_PROTO_STATIC)))
	{
		D("wan proto: dhcp or static.\n");
		dev_name = lookup_uci_option_str(ctx, uci_section, SLP_NETWORK_WAN_IFNAME_OPT);
	}
	else if (0 == strncmp(wan_proto, SLP_WAN_PROTO_PPPOE, strlen(SLP_WAN_PROTO_PPPOE)))
	{
		D("wan proto: pppoe.\n");
		dev_name = SLP_PPPOE_WAN_DEV_NAME;
	}
	strncpy(wan_conf->dev_name, dev_name ? dev_name : "", SLP_WAN_DEV_NAME_MAX_LEN);
	strncpy(wan_conf->wan_proto, wan_proto ? wan_proto : "", SLP_WAN_DEV_PROTO_MAX_LEN);

	/* get uci section: firewall.wan_manage */
	uci_section = get_uci_section(ctx, SLP_FIREWALL_PACKAGE_NAME, SLP_FIREWALL_WAN_MANAGE_SECT_NAME);
	if (NULL == uci_section)
	{
		D("find section(%s) failed\n", SLP_FIREWALL_WAN_MANAGE_SECT_NAME);
		goto wan_error;
	}

	/* get uci option: firewall.wan_manage.mode */
	listen_mode = lookup_uci_option_str(ctx, uci_section, SLP_FIREWALL_WAN_MODE_OPT);
	strncpy(wan_conf->listen_mode, listen_mode ? listen_mode : "", SLP_WAN_LISTEN_MODE_MAX_LEN);

	ret = 0;
wan_error:
	(void)free_uci_ctx(ctx);
	return ret;
}

static void *uh_release_server()
{
	D("Enter release server, free old wan listener...\n");
	pthread_detach(pthread_self());

	while (conf.old_wan_listener->n_clients)
	{
		D("listener(%d) were linked by %d clients, wait.\n",
			conf.old_wan_listener->fd.fd, conf.old_wan_listener->n_clients);
		sleep(1);
	}

	uh_release_listener(conf.old_wan_listener);
	return NULL;
}

static void uh_pthread_release()
{
	pthread_t release_thread;

	/* create thread to release old wan listener */
	if (0 != pthread_create(&release_thread, NULL, &uh_release_server, NULL))
	{
		D("pthread_create thread for releasing old wan listener failed.\n");
	}
}

static void uh_handle_rebind(int signo)
{
	int bound = 0;
	int release_server = 0;
	WAN_CONF_INFO wan_conf;
	...
	D("uhttpd receive a sigal[%s], rebind...\n", signo == SIGUSR1 ? "SIGUSR1" : "SIGUSR2");

	memset(&wan_conf, 0, sizeof(WAN_CONF_INFO));
	if (0 != get_wan_conf(&wan_conf))
	{
		D("get wan configure info failed or don't need to bind.\n");
		return ;
	}

	/* wan listenning was forbidden by firewall, or no port can used to listen */
	if (0 == strncmp(wan_conf.listen_mode, SLP_FIREWALL_WAN_FORBID, strlen(SLP_FIREWALL_WAN_FORBID))
		|| 	0 == strlen(wan_conf.listen_port))
	{
		if (SIGUSR2 == signo && conf.wan_listen_success)
		{
			conf.wan_listener->work = 0;
			conf.wan_listen_success = 0;
			conf.old_wan_listener = conf.wan_listener;
			uh_pthread_release();
		}

		D("wan device listen nothing.\n");
		return ;
	}

	/* wan device hasn't been changed, do not need to rebind */
	if ((SIGUSR1 == signo
		|| (SIGUSR2 == signo && 0 == strncmp(conf.old_wan_port, wan_conf.listen_port, strlen(wan_conf.listen_port)))
		)
		&& conf.wan_listen_success
		&& (0 == strncmp(wan_conf.wan_proto, SLP_WAN_PROTO_DHCP, strlen(SLP_WAN_PROTO_DHCP))
			|| 0 == strncmp(wan_conf.wan_proto, SLP_WAN_PROTO_STATIC, strlen(SLP_WAN_PROTO_STATIC)))
		&& 0 == strncmp(conf.old_dev_name, wan_conf.dev_name, strlen(wan_conf.dev_name))
		)
	{
		D("wan device hasn't been changed[%s], do not need to rebind.\n", wan_conf.dev_name);
		return ;
	}
	else
	{
		D("signal[%s], and wan port[%s->%s] or device[%s->%s] has been changed, rebind...\n",
			(SIGUSR2 == signo) ? "SIGUSR2" : "SIGUSR1",
			conf.old_wan_port, wan_conf.listen_port,
			conf.old_dev_name, wan_conf.dev_name);
	}

	conf.wan_dev_name = wan_conf.dev_name;
	if (conf.wan_listen_success)
	{
		release_server = 1;
		conf.wan_listener->work = 0;
		conf.wan_listen_success = 0;
	...
	D("wan info: host[%s], port[%s], proto[%s], dev[%s], mode[%s].\n", wan_conf.listen_host,
		wan_conf.listen_port, wan_conf.wan_proto, wan_conf.dev_name, wan_conf.listen_mode);

	bound = uh_socket_bind(wan_conf.listen_host[0] ? wan_conf.listen_host : NULL,
							wan_conf.listen_port, &hints, 0, &conf);
	if (bound < 1)
	{
		D("Error: No sockets bound, try again.\n");
		sleep(1);
		bound = uh_socket_bind(wan_conf.listen_host[0] ? wan_conf.listen_host : NULL,
							wan_conf.listen_port, &hints, 0, &conf);
		if (bound < 1)
		{
			fprintf(stderr, "Error: No sockets bound, give up.\n");
		}
	}
...
int main (int argc, char **argv)
{
...
#ifdef REMOTE_MANAGE
	sigHandle.sa_handler = uh_handle_rebind;
#else
	...
	while ((opt = getopt(argc, argv,
						 "fSDRC:K:E:I:p:s:h:c:l:L:d:r:m:n:x:i:t:T:A:u:U:w:")) > 0)
	{
		switch(opt)
		{
			/* [addr:]port */
