...
bool uh_lua_request(struct client *cl, lua_State *L)
{
	...
	case 0:
		...
		/* modified by wangfuyu, script name, cl->server->conf->lua_prefix */
		lua_pushstring(L, "");
		...
		{
			D("!!!!!!if (strlen(req->url) > strlen(prefix))\n");
			lua_pushstring(L, req->url);
			lua_setfield(L, -2, "PATH_INFO");
		}
...
#if 0
			else if (!strcasecmp(req->headers[i], "Accept"))
			{
				lua_pushstring(L, req->headers[i+1]);
