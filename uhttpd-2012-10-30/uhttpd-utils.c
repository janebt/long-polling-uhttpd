...
struct path_info * uh_path_lookup(struct client *cl, const char *url)
{
	...
	/* set path_phys as /www/cgi-bin/luci if /cgi-bin/luci was used */
	if (0 != strncmp(url, SLP_CGI_BIN_LUCI, strlen(SLP_CGI_BIN_LUCI))
		&& 0 != strncmp(url, SLP_LUCI_STATIC, strlen(SLP_LUCI_STATIC)))
	{
		memset(path_phys, 0, PATH_MAX);
		snprintf(path_phys, PATH_MAX, "%s%s", docroot, SLP_CGI_BIN_LUCI);
	}
...
int uh_listener_delete(struct listener * lis)
{
	struct listener *cur = NULL;
	struct listener *pre = NULL;

	for (pre = NULL, cur = uh_listeners; cur; pre = cur, cur = cur->next)
	{
		if (cur == lis)
		{
			if (pre)
...
void uh_client_timeout_cb(struct uloop_timeout *t)
{
	struct client *cl = NULL;
	
	cl = container_of(t, struct client, timeout);
	uh_client_remove(cl);
}
                              ...
                              struct sockaddr_in6 *peer)
{
	...
	if ((new = (struct client *)malloc(sizeof(struct client))) != NULL)
	{
		...
		new->timeout.cb = uh_client_timeout_cb;
		uloop_timeout_set(&new->timeout, no_data_timeout * 1000);
...
void uh_clean_client(struct listener *server)
{
	struct client *cur = NULL;
...
void uh_release_listener(struct listener *server)
{
	if (!server)
		return ;
	
	/* clients, which depend on server, will be cleaned */
	uh_clean_client(server);

	uh_ufd_remove(&server->fd);

	uh_listener_delete(server);
