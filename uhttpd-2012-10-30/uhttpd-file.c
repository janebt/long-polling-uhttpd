...
/* css/js/htm/html return false, else true */
static bool uh_file_cache(const char *url)
{
	if (strstr(url, ".css") || strstr(url, ".js") || strstr(url, ".htm"))
	{
		return false;
	}
	else
...
static int uh_file_response_ok_hdrs(struct client *cl, struct stat *s)
{
	...
	if (s)
	{
		...
		if (uh_file_cache(cl->request.url))
		{
			ensure_ret(uh_http_sendf(cl, NULL, "Cache-Control: %s\r\n", "max-age=86400"));
		}
		else
		{
			ensure_ret(uh_http_sendf(cl, NULL, "Cache-Control: %s\r\n", "no-cache"));
